...
Data Preprocessing
Model Training
Model Evaluation, Visualisation and Hyperparameter Tuning
...
Removes duplicate rows if any are present.
Fills missing numerical values in `LIMIT_BAL`, `AGE`, `PAY_AMT1` and
`PAY_AMT2` with each column's median.
Fills missing values in `SEX`, `EDUCATION` and `MARRIAGE` with each
column's mode.
Recalculates `LIMIT_BAL_LOG` using `numpy.log1p(LIMIT_BAL)`.
Investigates `risk_leak` and removes it from the processed dataset
because it may reveal information about the target.
Encodes `CITY` using scikit-learn's `LabelEncoder`.
Separates the target (`y`) from the input features (`X`).
Splits the data into training and testing sets with an 80/20 split,
`random_state=42`, and stratification on the target.
Standardises the features using `StandardScaler`, fitting the scaler
on the training data and applying it to the test data.
Applies SMOTE to the scaled training data to balance the target
classes.
...
Upload the `.ipynb` notebook to Google Colab or open it from your
repository.
Upload `Credit_Card.csv` to the Colab session, or mount storage and
update the CSV path in the dataset-loading cell.
Run the notebook cells in order from the beginning.
Review the printed evaluation tables and generated plots.
